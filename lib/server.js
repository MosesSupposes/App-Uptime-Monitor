    ...
    handleRequest(req, res) {
        ...
        req.on('end', function endBuffer() {
            ...
            try {
                ...
                chosenHandler(data, function routeRequestToHandler(statusCode=200, payload, contentType='json') {
                    // Call at the end of the event loop to avoid bug that would send two responses occasionally instead of one
                    setImmediate(this.processHandlerResponse.bind(this, res, method, trimmedPath, statusCode, payload, contentType));
                });
            } catch(e) {
                debug(e);
