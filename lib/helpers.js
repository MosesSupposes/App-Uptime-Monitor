...
const helpers = {
    ...
    trimFileExtensions(listOfFiles, properExtensions=['.json']) {
        return listOfFiles.reduce((files, file) => {
            let fileExtenstion = file.slice(file.indexOf('.'));
            let fileName = file.slice(0, -fileExtenstion.length);
    ...
    getTemplate(templateName='string', cb) {
        const validTemplateName = this.auditParam(templateName, 'string');
        if (validTemplateName) {
            const templatesDir = path.join(__dirname,'/../templates');
            fs.readFile(`${templatesDir}/${templateName}.html`, 'utf8', (err, str) => {
                (!err && this.auditParam(str)) ? 
                    cb(false, str)
