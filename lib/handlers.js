...
const _jsonSubMethods = {
                ...
                _data.read('users', phone, (err, userData) => {
                    ...
                    if (!err && userData) {
                        ...
                        if (passwordsMatch) {
                            ...
                            _performance.mark('beginning to store the token');
                            
                            // Store the token
                            _data.create('tokens', tokenId, tokenObj, err => {
                                _performance.mark('token storing complete');

                                const errMessage = 'Could not create the new token'
                                !err ? cb(200, tokenObj) : cb(500, {Error: errMessage});

                                //#region performance measurements
                                // Create a performance observer to log out all future measuremnts
                                const observer = new PerformanceObserver(list => {
                                    list.getEntriesByType('measure').forEach(logMeasurement);
                                });
                                ...
                                // Create measurements to be logged to debug console
                                ...
                                _performance.measure('Token data creation', 'creating data for the token', 'beginning to store the token');
                                _performance.measure('Token data storing', 'beginning to store the token', 'token storing complete');
                                
                                // Stop listening for measurement notifications
                                observer.disconnect()
                                //#endregion
                            });
                        } else {
    ...
    checks: {
        ...
        // Optional data: none
        delete(data, cb) {
            const { queryStringObject: { uid: id }, headers: { token } } = data;
            ...
            if (validId) {
                ...
                _data.read('checks', id, (err, checkData) => {
                    if (!err && checkData) {
                        ...
                        // Verify token 
                        _jsonSubMethods.tokens.verifyToken(token, userPhone, tokenIsValid => {
                            if (tokenIsValid) {
...
const _htmlSubMethods = {
    ...
    'checks/all': {
        get(data, cb) {
            ...
            // Respond with the template
            helpers.respondWithTemplate('checksList', templateData, cb);
        }
    },
    ...
    'checks/edit': {
        get(data, cb) {
            ...
            // Respond with the template
            helpers.respondWithTemplate('checksEdit', templateData, cb);
        }
    }
};
