...
function handleRequest(handler, data={}, acceptableRequests=allRequests, cb, statusCodeOnFail=405) {
    const { path, method: receivedRequest } = data;
    let subMethodToUse, subMethods;

    switch (handler.toLowerCase()) {
        case 'json': subMethodToUse = subMethods = _jsonSubMethods;
        case 'html': subMethodToUse = subMethods = _htmlSubmethods;
        default: subMethodToUse = subMethods = null;
    }

    acceptableRequests.includes(receivedRequest) && subMethodToUse ? 
        subMethods[path][receivedRequest](data, cb) 
    : cb(statusCodeOnFail, {Error: `this handler doesn't accept the specified request (${receivedRequest.toUpperCase()} or you passed an invalid handler: ${handler}`});
}
        ...
        if (data.method == 'get') {
            ...
            helpers.getTemplate('index', templateData, (err, template) => {
                if (!err && template) {
                    ...
                    helpers.addUniversalTemplates(template, templateData, (err, fullTemplate) => {
                        if (!err && fullTemplate) {
