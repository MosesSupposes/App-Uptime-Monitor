...
const _jsonSubMethods = {
    users: {
        ...
        console.log(auditRequiredFields(requiredFields))
...
function handleRequest(handler, data={}, acceptableRequests=allRequests, cb, statusCodeOnFail=405) {
    ...
    : cb(statusCodeOnFail, {Error: `this handler doesn't accept the specified path: (${path}), request: (${receivedRequest.toUpperCase()}), or you passed an invalid handler: (${handler})`});
}
