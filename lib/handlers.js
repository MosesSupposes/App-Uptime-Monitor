...
function handleRequest(data={}, acceptableRequests=allRequests, cb, statusCodeOnFail=405) {
    const { method: receivedRequest, path } = data;
    acceptableRequests.includes(receivedRequest) ? 
        _subMethods[path][receivedRequest](data, cb) 
    : cb(statusCodeOnFail);
}
...
const handlers = {
    ...
    // Handler for /users
    users(data, cb) {
        handleRequest(data, allRequests, cb);
    },
...
const _subMethods = {
    users: {
        ...
        get(data, cb) {
            ...
            if (validPhoneNumber) {
                ...
                _subMethods.tokens.verifyToken(token, phone, tokenIsValid => {
                    if (tokenIsValid) {
                        // Lookup the user
    ...
    checks: {
            ...
            if (validId) {
                ...
                _data.read('checks', id, (err, checkData) => {
                    if (!err && checkData) {
                        ...
                        _subMethods.tokens.verifyToken(token, checkData.userPhone, tokenIsValid => {
            ...
            const validInputs = auditRequiredFields([
                {
                    param: protocol,
                    requiredDataType: 'string',
                    compareOptions: () => ['https', 'http'].includes(protocol)
                },
                { 
                    param: url, 
                    requiredDataType: 'string', 
                ...
                },
                {
                    param: method,
                    requiredDataType: 'string',
                    compareOptions: () => ['get', 'post', 'put', 'delete'].includes(method)
                },
                { 
                    param: successCodes,
        ...
        // Required data: id: string
        // Optional data: protocol: string, url: string, method: string, successCodes: array (at least one must be set)
        put(data, cb) {
            const { auditParam, auditRequiredFields } = helpers;
            const { 
                payload: { 
            ...
            const validRemainingInputs = auditRequiredFields([
                { 
                    param: url, 
                    requiredDataType: 'string', 
                    compareOptions: null 
            ...
            const auditedOptionalFields = [validProtocol, validMethod, validTimeout, validRemainingInputs]
